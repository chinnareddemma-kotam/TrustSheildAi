...
// ─────────────────────────────────────────────────────────────────────────────
// Admin — Full AI Decision Explanation
// ─────────────────────────────────────────────────────────────────────────────

app.get('/api/admin/fraud-cases/:id/details',
  authenticateToken,
  requireRole('ADMIN'),
  (req, res) => {

    try {
      const fraudCase = db.prepare(`
        SELECT *
        FROM FraudCase
        WHERE id = ?
      `).get(req.params.id);
      ...
      // Get all AI/agent executions associated with this case
      const executions = db.prepare(`
        SELECT *
        FROM AgentExecution
        WHERE entityId = ?
        ORDER BY startedAt ASC
      `).all(fraudCase.entityId);
      ...
      // Find the most relevant AI execution
      ...
      return res.json({
        ...
        aiDecision: {
          ...
          riskScore: fraudCase.riskScore ?? null,
          agent: fraudCase.agent || null,
          modelVersion: fraudCase.modelVersion || 'v2.4.0',
          explanation:
            fraudCase.explanation ||
            fraudCase.reason ||
            'No AI explanation was stored for this case.'
        },

        policy: {
          policyName: fraudCase.policy || null,
          ruleTriggered: fraudCase.ruleTriggered || null,
          reason:
            fraudCase.explanation ||
            fraudCase.reason ||
            'No policy explanation was stored.'
        },
        ...
        humanDecision: {
          decision:
            fraudCase.overrideDecision ||
            fraudCase.status ||
            null,
          reason: fraudCase.overrideReason || null,
          overridden:
            !!fraudCase.overrideReason
        }
      });
      ...
      console.error(
        'Failed to load fraud case details:',
        error
      );

      return res.status(500).json({
        error: 'Failed to load fraud case details'
      });
    }
  }
);
...
// Dataset Registry — checks for real dataset files on disk; does NOT fabricate metrics
app.get('/api/admin/dataset-registry', authenticateToken, requireRole('ADMIN'), (req, res) => {
  const dataRawDir = path.join(__dirname, 'data', 'raw');

  function checkDatasetPresent(filename) {
    const fullPath = path.join(dataRawDir, filename);
    return fs.existsSync(fullPath);
  }

  // Retrieve model versions from DB for adapter status
  const modelVersions = db.prepare('SELECT * FROM ModelVersion').all();
  function getModelVersion(agentName) {
    return modelVersions.find(m => m.agentName === agentName) || null;
  }
  ...
  const registry = [
    {
      ...
      purpose: 'Transaction fraud detection: evaluates IP velocity, COD abuse, V1-V339 anonymous features, device fingerprinting',
      sourceUrl: 'https://www.kaggle.com/c/ieee-fraud-detection/data',
      expectedFile: 'ieee_cis_fraud.csv',
      configStatus: checkDatasetPresent('ieee_cis_fraud.csv') ? 'CONFIGURED' : 'NOT CONFIGURED',
      adapterStatus: 'DEMO AI ADAPTER — DATASET NOT CONFIGURED',
      modelVersion: getModelVersion('Risk Scoring Agent')?.version || 'v2.4.0 (Demo)',
      evaluationStatus: 'EVALUATION PENDING',
      evaluationNote: 'Accuracy, precision, recall, and F1 metrics are available only after training on this dataset.',
      requiredColumns: ['TransactionID','isFraud','TransactionAmt','ProductCD','card1..card6','addr1','addr2','P_emaildomain','R_emaildomain','C1..C14','D1..D15','M1..M9','V1..V339'],
    },
    {
      id: 'ds-kaggle-ecomm',
      name: 'Kaggle E-Commerce Fraud Dataset',
      agent: 'Risk Scoring Agent',
      purpose: 'COD doorstep refusal history, buyer repeat fraud patterns, return abuse signals',
      sourceUrl: 'https://www.kaggle.com/datasets/vbinh002/fraud-ecommerce',
      expectedFile: 'ecommerce_fraud.csv',
      configStatus: checkDatasetPresent('ecommerce_fraud.csv') ? 'CONFIGURED' : 'NOT CONFIGURED',
      adapterStatus: 'DEMO AI ADAPTER — DATASET NOT CONFIGURED',
      modelVersion: getModelVersion('Risk Scoring Agent')?.version || 'v2.4.0 (Demo)',
      evaluationStatus: 'EVALUATION PENDING',
      evaluationNote: 'Accuracy, precision, recall, and F1 metrics are available only after training on this dataset.',
      requiredColumns: ['purchase_value','purchase_time','signup_time','device_id','source','browser','sex','age','ip_address','class'],
    },
    {
      id: 'ds-amazon-reviews',
      name: 'Amazon Fake Reviews Dataset',
      agent: 'Review Moderation Agent',
      purpose: 'Fake, paid, and AI-generated review detection via text perplexity and verified-purchase signal',
      sourceUrl: 'https://www.kaggle.com/datasets/lievgarcia/amazon-reviews',
      expectedFile: 'amazon_fake_reviews.csv',
      configStatus: checkDatasetPresent('amazon_fake_reviews.csv') ? 'CONFIGURED' : 'NOT CONFIGURED',
      adapterStatus: 'DEMO AI ADAPTER — DATASET NOT CONFIGURED',
      modelVersion: getModelVersion('Review Moderation Agent')?.version || 'v3.1.0 (Demo)',
      evaluationStatus: 'EVALUATION PENDING',
      evaluationNote: 'Accuracy, precision, recall, and F1 metrics are available only after training on this dataset.',
      requiredColumns: ['label','text_','title','images','rating','verified'],
    },
    {
      id: 'ds-opspam',
      name: 'OpSpam Corpus (Opinion Spam)',
      agent: 'Review Moderation Agent',
      purpose: 'Deceptive opinion spam detection: truthful vs fabricated hotel/product review classification',
      sourceUrl: 'https://myleott.com/op-spam.html',
      expectedFile: 'opspam_corpus.csv',
      configStatus: checkDatasetPresent('opspam_corpus.csv') ? 'CONFIGURED' : 'NOT CONFIGURED',
      adapterStatus: 'DEMO AI ADAPTER — DATASET NOT CONFIGURED',
      modelVersion: getModelVersion('Review Moderation Agent')?.version || 'v3.1.0 (Demo)',
      evaluationStatus: 'EVALUATION PENDING',
      evaluationNote: 'Accuracy, precision, recall, and F1 metrics are available only after training on this dataset.',
      requiredColumns: ['label','review_text','hotel_name','reviewer_id','date'],
    },
    {
      id: 'ds-counterfeit-logo',
      name: 'Counterfeit Product & Logo Detection Dataset',
      agent: 'Authenticity Agent',
      purpose: 'Logo geometry matching, brand image authentication, unauthorized product detection',
      sourceUrl: 'https://www.kaggle.com/datasets/search?q=counterfeit+product+logo+detection',
      expectedFile: 'counterfeit_logos.csv',
      configStatus: checkDatasetPresent('counterfeit_logos.csv') ? 'CONFIGURED' : 'NOT CONFIGURED',
      adapterStatus: 'DEMO AI ADAPTER — DATASET NOT CONFIGURED',
      modelVersion: getModelVersion('Authenticity Agent')?.version || 'v1.8.2 (Demo)',
      evaluationStatus: 'EVALUATION PENDING',
      evaluationNote: 'Accuracy, precision, recall, and F1 metrics are available only after training on this dataset.',
      requiredColumns: ['product_image_url','brand','is_counterfeit','logo_match_score','authorization_doc'],
    },
    {
      id: 'ds-innv-luxury',
      name: 'INNV Luxury Fashion Fraud Detection Dataset',
      agent: 'Authenticity Agent',
      purpose: 'MSRP variance analysis, luxury brand unauthorized listings, serial number pattern validation',
      sourceUrl: 'https://github.com/INNV-AI/luxury-fraud-detection',
      expectedFile: 'innv_luxury_fraud.csv',
      configStatus: checkDatasetPresent('innv_luxury_fraud.csv') ? 'CONFIGURED' : 'NOT CONFIGURED',
      adapterStatus: 'DEMO AI ADAPTER — DATASET NOT CONFIGURED',
      modelVersion: getModelVersion('Authenticity Agent')?.version || 'v1.8.2 (Demo)',
      evaluationStatus: 'EVALUATION PENDING',
      evaluationNote: 'Accuracy, precision, recall, and F1 metrics are available only after training on this dataset.',
      requiredColumns: ['listing_title','brand','price_usd','msrp_usd','has_auth_document','is_fraud'],
    },
  ];
  ...
  return res.json({
    registry,
    dataRawDir,
    note: 'Place dataset CSV files in the data/raw/ directory using the expectedFile filename to configure a dataset. The application will automatically detect presence and update configStatus.',
  });
});
